...
INDEXES_DIR = SCRIPT_DIR.parent.joinpath("indexes", "server", "common")
MANIFESTS_DIR = SCRIPT_DIR.parent.joinpath("manifests", "server", "common")

INDEXES_URL = "https://github.com/aoqia194/leaf/raw/refs/heads/main/indexes/server/common"
LAUNCHER_MANIFEST_URL = "https://github.com/aoqia194/leaf/raw/refs/heads/main/manifests/server/common"

VERSION_TABLE = {
    "7269353596609305518": "41.78.16",
}
...
print("Latest Zomboid Dedicated Server (Content Depot) versions according " +
      f"to script (update if needed):\nLatest = {LATEST_VER}\nSnapshot = {LATEST_SNAPSHOT}")
...
def main():
    ...
    snap_ver = VERSION_SNAPSHOT_TABLE.get(manifest_id)
    ver = VERSION_TABLE.get(manifest_id)
    manifest_version = snap_ver if snap_ver is not None else (ver if ver is not None else (None))
    ...
    if not version_manifest.exists():
        ...
        try:
            ...
            print("Invalid text data found in manifest file, overwriting.")
            manifests_data["versions"].append(manifest_data_table)
        except json.JSONDecodeError:
            print("Invalid json data found in manifest file, overwriting.")
            manifests_data["versions"].append(manifest_data_table)
        finally:
            f.close()

    json_data = json.dumps(manifests_data)
